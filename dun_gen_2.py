# Global variables for map characters
WALL_CHAR = "#"
FLOOR_CHAR = " "
EMPTY_CHAR = "Ç"
DOOR_OPEN_CHAR = "="
...
class dMap:
    ...
    def make_map(self, xsize, ysize, fail, b1, mrooms):
        ...
        # make_map can be modified to accept arguments for values of failed, and percentile of features.
        # Create first room
        self.size_x = xsize
        self.size_y = ysize
        ...
        for y in range(ysize):
            ...
            for x in range(xsize):
                tmp.append(1)
            self.map_arr.append(tmp)

        w, l, t = self.make_room()
        while len(self.room_list) == 0:
            y = randrange(1, ysize - l)
            x = randrange(1, xsize - w)
            p = self.place_room(l, w, x, y, xsize, ysize, 6, 0)
        failed = 0
        while failed < fail:  # The lower the value that failed< , the smaller the dungeon
            choose_room = randrange(len(self.room_list))
            ...
            if room_done == 0:  # If placement failed increase possibility map is full
                ...
                failed = 0
                if t < 5:
                    tc = [len(self.room_list) - 1, ex2, ey2, t]
                    self.c_list.append(tc)
                    self.join_corridor(len(self.room_list) - 1, ex2, ey2, t, 50)
            if len(self.room_list) == mrooms:
                failed = fail
        self.final_joins()
    ...
    def make_exit(self, rn):
        ...
        while True:
            ...
            if rw == 0:  # North wall
                ...
                rx2 = rx - 1
                ry2 = ry
            if self.map_arr[ry][rx] == 2:  # If space is a wall, exit
                break
    ...
    def join_corridor(self, cno, xp, yp, ed, psb):
        ...
        for xxx, yyy, xxx1, yyy1 in check_exit:  # Loop through possible exits
            if self.map_arr[yyy][xxx] == 0:  # If joins to a room
...
# ----------------------------------------------------------------------------
startx = 65  # map width
starty = 20  # map height
themap = dMap()
themap.make_map(startx, starty, 110, 50, 60)
for y in range(starty):
    ...
    for x in range(startx):
        ...
        if themap.map_arr[y][x] == 2:
            line += WALL_CHAR
        if themap.map_arr[y][x] == 3:
            line += DOOR_OPEN_CHAR
        if themap.map_arr[y][x] == 4:
            line += DOOR_CLOSED_CHAR
        if themap.map_arr[y][x] == 5:
            line += DOOR_SECRET_CHAR
        if themap.map_arr[y][x] == 6:
            line += f"{Fore.BLACK + Style.BRIGHT}{EMPTY_CHAR}{Style.RESET_ALL}"
    print(line)
